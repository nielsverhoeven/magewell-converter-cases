...
### Fan power, thermal switch, and manual switch (`fan=true` only)
...
| MTS-101 SPST ON-OFF mini toggle switch, black, panel-mount | MTS-101 (or equivalent, ⌀6.4 mm mounting hole, ~8 mm AF nut) | 1 | **Plus-family only** (`pro-convert-hdmi-plus`, `pro-convert-sdi-plus`, `pro-convert-for-ndi-to-hdmi-4k`) — the compact family's feasible `switch_y` band is empty at any clearance (architect ruling D-18, `.claude/knowledge/layout-patch-wall.md` §18.2); do not fit on a compact SKU even with `-D fan=true`. Wired in series in the fan's +5V lead, **default: switch only (wiring variant (a))** — optionally add the KSD9700 above in series for variant (b), master switch + thermostat; the user has not yet confirmed which variant ships by default (`.claude/knowledge/architecture.md` §12 Q19). Fully recessed (~11 mm well, `MCC_SWITCH_WELL_DEPTH_MAX` guard) into a locally-thickened +X wall pad beside the fan aperture, so nothing protrudes and no envelope figure moves (D-13 pattern). Every figure — actuator height, nut across-flats, body depth, panel-clamp range — is `assumed` pending **M17** (buy one, ≈€1–2, and measure before the first full-size print); DC switching at 5 V/≤0.05 A is unverified, same open question as the KSD9700 above | This plan/ticket #32 §1; `.claude/knowledge/architecture.md` §5, §10 D-18, §11 R29, §12 M17; `knowledge/components/switches.md` (new) |
| KUOQIY USB-A → 3/4-pin PWM fan adapter cable | KUOQIY, 5-pack (amazon.nl ASIN B0D1C36WKB), 5 V, 30 cm, black | 1 (of 5) | **Decoders only** (NDI to HDMI, NDI to SDI, NDI to HDMI 4K). Plugs straight into the `NF-A4x10 5V`'s own native 3-pin connector — **no OmniJoin/NA-AC2 adaptor needed** for this specific cable (see the OmniJoin row below). Runs from the device's external `usb_host` port, through the DBA-BL-B-blanked slot, through the manual switch above (Plus-family SKUs only — n/a for these compact decoders) and/or the thermoswitch, to the fan — rating under PoE is `unknown` and needs measurement M9 before relying on it | `knowledge/components/fans.md` (KUOQIY 3/4-pin note, new); issue #32 (cable spec, amazon.nl, fetched 2026-09-09 — not independently re-verified this pass, amazon fetches return HTTP 500); `knowledge/components/fan-power-sources.md:21,39-46`; `.claude/knowledge/architecture.md` §5 table row "Decoders ... R21, M9" |
| Cut-down Mini-DIN-8 male pigtail | generic, wired to pin 8 VCC / pin 4 GND only | 1 | **Encoders only** (HDMI Plus, SDI Plus; TX if a fan is ever fitted). Runs from the device's internal (`panel:"none"`) Mini-DIN-8 "PTZ+TALLY" port, pin 8 VCC (5 V, 100 mA max) / pin 4 GND, through the manual switch and/or the thermoswitch above, to the fan — entirely inside the case. **Not** a splice onto the `usb_b` +5V feed (deviation D17: no 5 V there under PoE). The fan's 50 mA is half the port's 100 mA budget before inrush (R22, M10 — measure before relying on it) | `knowledge/components/fan-power-sources.md:80` (pin 8 rating), `:99-106` (Tally/matrix share the same budget); `knowledge/components/mini-din8-feedthrough.md:38`; `.claude/knowledge/architecture.md` §5 table row "Encoders ... R22, M10" |
...
| OmniJoin adaptor set / NA-AC2 3:2-pin adaptor cable | bundled with the fan | 1 | Ships **in the box** with every `NF-A4x10 5V` — **not needed for the KUOQIY cable path** above (its 3/4-pin plug mates directly with the NF-A4x10 5V's own connector); this row now documents only the bundled-but-unused accessory, kept so it isn't mistaken for missing hardware | `knowledge/components/fans.md:13-24` ("ships with NA-AC2 3:2-pin adaptor cable, OmniJoin adaptor set") |
...
| MTS-101 SPST ON-OFF mini toggle switch, black, panel-mount | 1 | One of the 3 Plus-family SKUs the manual fan switch ships on (rev 11, #32, D-18 — the compact family's fit is infeasible). Fully recessed beside the fan aperture on the +X wall, wired in series in the fan's +5V lead upstream of the KSD9700 below; every figure `assumed` pending M17 | `.claude/knowledge/architecture.md` §5, §10 D-18, §12 M17; `knowledge/components/switches.md` (new) |
| KSD9700 45 °C normally-open bimetal thermal switch | 1 | Wired in series between the fan's +5V lead and the source below, downstream of the manual switch above, bonded to the device's metal top so the fan only runs once the device is actually hot; body + pad ≤ 8.8 mm tall (M11); **DC switching at 5 V/≤0.05 A is unverified — every published rating is 250 V AC/5–16 A (M8, bench-test before relying on it)** | `.claude/knowledge/architecture.md` §5 "Fan power is device-sourced (D-14)"; `knowledge/components/poe-splitter-verification.md:151-196` |
| KUOQIY USB-A → 3/4-pin PWM fan adapter cable (5-pack, amazon.nl ASIN B0D1C36WKB) | 1 (of 5) | Power source is **the device's own external `usb_host` port**, internally cabled from the blanked DBA-BL-B slot through the manual switch and thermoswitch above to the fan — NOT a splice onto the `usb_b` power feed (that reasoning applies to the encoders, D17, see the HDMI Plus/SDI Plus sections above). Plugs straight into the `NF-A4x10 5V`'s own native 3-pin connector, no OmniJoin/NA-AC2 adaptor needed; the decoder's USB-A host port is a plausible 5 V source but its rating under PoE is **not stated by Magewell** and needs measurement M9 before relying on it | `knowledge/components/fans.md` (KUOQIY 3/4-pin note, new); `knowledge/components/fan-power-sources.md:21,39-46` (host-port rating `unknown`); `.claude/knowledge/architecture.md` §5 table row "Decoders ... device's USB-A host port ... unknown — R21, M9"; issue #32 |
| HDMI patch cable, straight plug, 0.3 m | 1 | `hdmi_out` | `knowledge/components/cables.md:54,121` |
| USB 2.0 A-to-B cable, 0.15 m | 1 | `usb_b` | `knowledge/components/cables.md:79,125` |
| Cat6 slim RJ45 patch cable, 0.15 m | 1 | `rj45` | `knowledge/components/cables.md:23,119` |
