//////////////////////////////////////////////////////////////////////
// models/pro-convert-hdmi-plus/case.scad
//   L4 thin assembly (architecture.md §4, new-case-variant skill), copied from the normative
//   template models/pro-convert-for-ndi-to-hdmi/case.scad. First Plus-family case (issue #5).
//   part in {"base","lid","panel","assembly","panel_placed","ghost_device","ghost_plugs"}; only
//   base/lid/panel are picked up by scripts/build.py's discover_models() by default (it hardcodes
//   ["base","lid"] + "panel" iff the literal substring 'part == "panel"' appears below). Unlike the
//   NDI-to-HDMI template, this file does NOT declare a "base_fan" extra_parts marker: this SKU's
//   `fan` variant flag already defaults to true (see below), so "base" already IS the fan-fitted
//   mesh -- a separate "base_fan" part would just re-render the same geometry under a second name.
// Render:
//   openscad --backend=Manifold -D 'part="base"' -o out/base.stl models/pro-convert-hdmi-plus/case.scad
//////////////////////////////////////////////////////////////////////
...
// from this case, remove/edit it in lib/mcc/devices/pro-convert-hdmi-plus.scad instead
...
//               DEFAULTS TO true ON THIS SKU (unlike the NDI-to-HDMI template's false): the Plus
//               chassis' ~10 W thermal budget makes the fan not optional per
//               .claude/knowledge/layout-patch-wall.md §16.2 item 4 / architecture.md §11 R5, and
//               the user made this an explicit per-SKU decision on 2026-09-08 (risk R5) rather than
//               leaving the three Plus-family SKUs to silently inherit the compact family's
//               fan=false default. Fed from the top-level `fan` variable below so `-D fan=false`
//               still lets a reviewer render the fan-less shell for comparison.
...
echo(str("pro-convert-hdmi-plus: L=", L_dims[0], " W=", L_dims[1], " H=", L_dims[2]));
echo(str("pro-convert-hdmi-plus: slots=", mcc_slot_assignment(dev)));
