...
//   §16, GitHub issue #3). part in {"base","lid","panel","assembly","ghost_device","ghost_plugs"};
//   only base/lid/panel are ever picked up by scripts/build.py (discover_models() hardcodes
//   ["base","lid"] + "panel" iff the literal substring 'part == "panel"' appears below).
// Render:
//   openscad --backend=Manifold -D 'part="base"' -o out/base.stl models/pro-convert-hdmi-tx/case.scad
//////////////////////////////////////////////////////////////////////
...
// from this case, remove/edit it in lib/mcc/devices/pro-convert-hdmi-tx.scad instead
...
dev = MCC_DEV_PRO_CONVERT_HDMI_TX;
...
echo(str("pro-convert-hdmi-tx: L=", L_dims[0], " W=", L_dims[1], " H=", L_dims[2]));
echo(str("pro-convert-hdmi-tx: slots=", mcc_slot_assignment(dev)));
